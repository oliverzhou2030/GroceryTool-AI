...
- Receipt-structure filtering for addresses, quantity/tare details, sales summaries, payment lines, and return-policy text
- Refundable container deposits kept as distinct dark-green line items and excluded from food-category charts
...
The Mac launcher connects to the `iPhone 17 Pro` Simulator data container. Receipt history, original images, PDFs, accounts, reviews, and learned preferences then refresh automatically in both running apps. If Xcode has removed the Simulator app registration, `./run-mac.command` automatically reinstalls it and recovers the largest existing GroceryTool AI receipt store and its document files before opening either app.
...
Open the separate **Tools** tab for **Grocery Budget**, **Ask Grocery AI**, and **Settings**. Grocery Budget accepts an amount, period length, and days/weeks/months; the Shop screen uses the saved limit when ranking plans. To enable Ask Grocery AI, put a complete DeepSeek API key on one line in `.deepseek-key` at the repository root and restart with either launcher; the ignored key file is never committed. Only chat messages are sent to DeepSeek—receipt images and receipt history are not attached.
