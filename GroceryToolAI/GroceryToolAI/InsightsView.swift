    ...
    var body: some View {
        NavigationStack {
            ScrollView { VStack(spacing: 20) {
                GroupBox("Analysis period") { VStack(spacing: 12) { DatePicker("From", selection: $start, in: ...end, displayedComponents: .date); DatePicker("Through", selection: $end, in: start..., displayedComponents: .date); HStack { Label("Every total, category count, and ratio below uses this date range.", systemImage: "calendar.badge.checkmark").font(.footnote).foregroundStyle(.secondary); Spacer(); if let url = store.exportURL(for: analytics.receipts) { ShareLink(item: url) { Label("Export CSV", systemImage: "square.and.arrow.up") }.buttonStyle(.borderedProminent) } } } }.frame(maxWidth: .infinity)
