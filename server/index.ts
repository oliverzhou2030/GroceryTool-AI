...
app.post('/api/receipts/scan', upload.array('receipts', 20), async (req, res) => {
  try {
    if (!process.env.OPENAI_API_KEY) return res.status(503).json({ error: 'OPENAI_API_KEY is not configured. Copy .env.example to .env and add your key.' });
    ...
    const receipts = await Promise.all(files.map(async (file) => {
      ...
      const media = file.mimetype === 'application/pdf'
        ? { type: 'input_file' as const, filename: file.originalname, file_data: dataUrl }
